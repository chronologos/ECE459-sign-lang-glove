...
void mrf_init(uint8_t channel)
{
    ...
    mrf_spi_init(8, 0, 5000000);     // 8-bit transfers, CPOL=0/CPHA=0, sclk = 5MHz
...
void mrf_reset(uint8_t channel)
{
	...
	mrf_write_short(TXSTBL, 0x93); // Vuk: This is a recommended value

  // Enable and configure radio
  mrf_write_long(RFCON0, 0x03);   // Vuk: This was changed from 0xE3 to 0x03 as per datasheet
  mrf_write_long(RFCON1, 0x02);   // Set VCO param to recommended value
  ...
  //  mrf_write_short(WAKECON, 0x80); // Enable immediate wake-up
	...
	#ifdef MRF24J40MA
		mrf_write_long(RFCON3, 0x00); // Max Tx power MRF24J40MA
	#endif
	#ifdef MRF24J40MD
		mrf_write_long(RFCON3, 0x28); // Max Tx power MRF24J40MD
	#endif
...
{
    mrf_write_long(TESTMODE, 0x08 | 5);    // Put into single-tone test mode
...
void mrf_data_mode(void)	 // Vuk: This function is not used
{
    ...
    mrf_write_long(TESTMODE, 0x08);        // Put back into default mode
    nrk_spin_wait_us(500);
...
{         	
    uint8_t rssi; 
//	#ifdef MRF24J40MD
//	mrf_write_long(TESTMODE, 0x08);
//	mrf_write_short(TRISGPIO, mrf_read_short(TRISGPIO) | 0x06);
//	mrf_write_short(GPIO, (mrf_read_short(GPIO) | 0x40) & ~0x20);
//	#endif

	
    mrf_write_short(BBREG6, 0x80);
    
    // Wait for result to be ready
    while(!(mrf_read_short(BBREG6) & 0x01));
    rssi = mrf_read_long(RSSI);
    
    mrf_write_short(BBREG6, 0x40);
	
//	#ifdef MRF24J40MD
//	mrf_write_long(TESTMODE, 0x0F);
//	#endif
    return rssi;
}
