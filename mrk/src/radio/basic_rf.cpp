    ...
    uint16_t cnt = 0;
		while(!tx_status_ready)
		{
			...
			{					
				...
				#ifdef _RT_LINK_H
					rf_init(&rtl_rfRxInfo, RADIO_CHANNEL, 0xFFFF, 0); //RX struct, channel, panId, myAddr
				#elifdef _BMAC_H
					rf_init(&bmac_rfRxInfo, RADIO_CHANNEL, 0xFFFF, 0); //RX struct, channel, panId, myAddr
				#else
